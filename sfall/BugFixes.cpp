...
static void __declspec(naked) obj_load_func_hack() {
	__asm {
		test word ptr [eax + 0x24], 0x400; // ObjectFlag Temp (engine code)
		jz   fix;
		retn;
fix:
		mov  edi, [eax + 0x64];
...
static void __declspec(naked) combat_ctd_init_hack() {
	...
	__asm {
		...
		mov  [ebx + 0x54], eax;                   // pobj.who_hit_me = 0
end:
...
static void __declspec(naked) obj_move_to_tile_hack() {
	...
	__asm {
		cmp  ds:[_loadingGame], 0; // prevents leaving the map right after loading a saved game if last time the player died on another map
		jnz  skip;
