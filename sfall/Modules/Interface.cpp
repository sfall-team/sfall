...
static void __fastcall DrawAmmoBar(long barX, long barFillHeight) {
	fo::Window* win = fo::func::GNW_find(*fo::ptr::interfaceWindow);
	if (win == nullptr) return;
	...
	rect.right = barX + 2;
	rect.bottom = 26 + 70;
	fo::func::win_draw_rect(*fo::ptr::interfaceWindow, &rect);
}
...
void Interface::init() {
	...
	if (!hrpIsEnabled || IniReader::GetInt("IFACE", "ALTERNATE_AMMO_METRE", 0, ".\\f2_res.ini") == 0) {
