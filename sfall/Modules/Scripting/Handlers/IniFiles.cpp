...
void mf_get_ini_sections(OpcodeContext& ctx) {
	...
	size_t i = 0;
	for (Config::Data::const_iterator sectIt = data.cbegin(); sectIt != data.cend(); ++sectIt) {
...
static void CopyConfigSectionToArray(DWORD arrayId, const Config::Section& section) {
	for (Config::Section::const_iterator valueIt = section.cbegin(); valueIt != section.cend(); ++valueIt) {
		SetArray(arrayId, valueIt->first.c_str(), valueIt->second.c_str(), false);
	}
}

void mf_get_ini_section(OpcodeContext& ctx) {
	const char* sectionName = ctx.arg(1).strValue();
	...
	const Config::Data& data = config->data();
	Config::Data::const_iterator sectIt = data.find(sectionName);
...
void mf_get_ini_config(OpcodeContext& ctx) {
	...
	if (isDb) {
		config = new Config();
		if (!config->read(filePath.c_str(), isDb)) {
	...
	cache.insert(std::make_pair(std::move(filePath), arrayId));
	ctx.setReturn(arrayId);

	if (isDb) delete config;
}
