...
void op_create_array(OpcodeContext& ctx) {
	DWORD arrayId = CreateArray(ctx.arg(0).rawValue(), ctx.arg(1).rawValue());
...
void op_temp_array(OpcodeContext& ctx) {
	const ScriptValue& flags = ctx.arg(1);
	...
	DWORD arrayId = CreateTempArray(ctx.arg(0).rawValue(), flags.rawValue());
	ctx.setReturn(arrayId);
}
...
void op_list_as_array(OpcodeContext& ctx) {
	DWORD arrayId = ListAsArray(ctx.arg(0).rawValue());
	ctx.setReturn(arrayId);
}
