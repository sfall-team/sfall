...
void mf_spatial_radius(OpcodeContext& ctx) {
	fo::GameObject* spatialObj = ctx.arg(0).object();
...
void op_get_script(OpcodeContext& ctx) {
	long scriptIndex = ctx.arg(0).object()->scriptIndex;
...
void op_make_straight_path(OpcodeContext& ctx) {
	fo::GameObject* objFrom = ctx.arg(0).object();
...
void op_make_path(OpcodeContext& ctx) {
	fo::GameObject* objFrom = ctx.arg(0).object();
	DWORD tileTo = ctx.arg(1).rawValue();
	DWORD func = getBlockingFunc((BlockType)ctx.arg(2).rawValue());
...
void op_get_party_members(OpcodeContext& ctx) {
	DWORD includeHidden = ctx.arg(0).rawValue();
...
void mf_car_gas_amount(OpcodeContext& ctx) {
	ctx.setReturn(*fo::ptr::carGasAmount);
}
...
void mf_obj_under_cursor(OpcodeContext& ctx) {
	ctx.setReturn(
		fo::func::object_under_mouse(ctx.arg(0).asBool() ? 1 : -1, ctx.arg(1).rawValue(), *fo::ptr::map_elevation)
	);
}

void mf_get_loot_object(OpcodeContext& ctx) {
	ctx.setReturn((GetLoopFlags() & INTFACELOOT) ? fo::ptr::target_stack[*fo::ptr::target_curr_stack] : 0);
}
