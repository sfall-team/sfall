...
static void mf_get_metarule_table(OpcodeContext& ctx) {
	...
	ctx.setReturn(arrId, DATATYPE_INT);
}

static void mf_metarule_exist(OpcodeContext& ctx) {
	bool result = false;
	const char* funcXName = ctx.arg(0).asString();
	if (funcXName[0] != '\0') {
...
void InitMetaruleTable() {
	for (const SfallMetarule* metarule = std::begin(metarules); metarule != std::end(metarules); ++metarule) {
