...
static void __declspec(naked) get_mouse_x() {
   __asm {
	  ...
	  push edx;
	  mov ecx, eax;
	  mov edx, ds:[VarPtr::mouse_x_];
	  add edx, ds:[VarPtr::mouse_hotx];
	  call FuncOffs::interpretPushLong_;
	  mov eax, ecx;
	  mov edx, 0xc001;
	  call FuncOffs::interpretPushShort_
...
static void __declspec(naked) get_mouse_y() {
   __asm {
	  ...
	  push edx;
	  mov ecx, eax;
	  mov edx, ds:[VarPtr::mouse_y_];
	  add edx, ds:[VarPtr::mouse_hoty];
	  call FuncOffs::interpretPushLong_;
	  mov eax, ecx;
	  mov edx, 0xc001;
	  call FuncOffs::interpretPushShort_
...
static void __declspec(naked) get_mouse_buttons() {
   __asm {
	  ...
	  push edx;
	  mov ecx, eax;
	  mov edx, ds:[VarPtr::last_buttons];
	  call FuncOffs::interpretPushLong_;
	  mov eax, ecx;
	  mov edx, 0xc001;
	  call FuncOffs::interpretPushShort_
...
static void __declspec(naked) get_window_under_mouse() {
   __asm {
	  ...
	  push edx;
	  mov ecx, eax;
	  mov edx, ds:[VarPtr::last_button_winID];
	  call FuncOffs::interpretPushLong_;
	  mov eax, ecx;
	  mov edx, 0xc001;
	  call FuncOffs::interpretPushShort_
...
static void __declspec(naked) get_screen_width() {
	__asm {
		push edx
		push eax
		mov  edx, ds:[VarPtr::scr_size+8]                // _scr_size.offx
		sub  edx, ds:[VarPtr::scr_size]                  // _scr_size.x
		inc  edx
		call FuncOffs::interpretPushLong_
		pop  eax
		mov  edx, 0xc001
...
static void __declspec(naked) get_screen_height() {
	__asm {
		push edx
		push eax
		mov  edx, ds:[VarPtr::scr_size+12]               // _scr_size.offy
		sub  edx, ds:[VarPtr::scr_size+4]                // _scr_size.y
		inc  edx
		call FuncOffs::interpretPushLong_
		pop  eax
		mov  edx, 0xc001
...
static void __declspec(naked) create_message_window() {
   __asm {
	  pushad
	  mov ebx, dword ptr ds:[VarPtr::curr_font_num];
	  cmp ebx, 0x65;
	  je end;
...
static void __declspec(naked) GetViewportX() {
	__asm {
		...
		push edx;
		mov ecx, eax;
		mov edx, ds:[VarPtr::wmWorldOffsetX];
		call FuncOffs::interpretPushLong_;
		mov edx, 0xc001;
		mov eax, ecx;
		call FuncOffs::interpretPushShort_;
...
static void __declspec(naked) GetViewportY() {
	__asm {
		...
		push edx;
		mov ecx, eax;
		mov edx, ds:[VarPtr::wmWorldOffsetY];
		call FuncOffs::interpretPushLong_;
		mov edx, 0xc001;
		mov eax, ecx;
		call FuncOffs::interpretPushShort_;
