...
void Input::init() {
	...
	if (xltKey > 0) {
		std::vector<std::string> codeList = Translate::GetList("sfall", "XltTable", "", ',');
		...
		if (numCodes > 0) {
			...
			const DWORD charCodeMaxAddr[] = {0x433ED6, 0x47F2F7};
			SafeWriteBatch<BYTE>(0x7E, charCodeMaxAddr); // max ASCII code value allowed to input (was 122)
		}
	}
}
