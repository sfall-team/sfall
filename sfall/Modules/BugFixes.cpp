...
static void __fastcall CheckAddiction(fo::GameObject* critter) {
	...
	for (size_t i = 0; i < 9; i++) {
		long pid = fo::ptr::drugInfoList[i].itemPid;
...
static void __stdcall combat_attack_gcsd() {
	...
	if ((*fo::ptr::main_ctd).attackerFlags & fo::DamageFlag::DAM_HIT) {
		...
		if ((*fo::ptr::main_ctd).targetDamage < (*fo::ptr::gcsd)->minDamage) {
			(*fo::ptr::main_ctd).targetDamage = (*fo::ptr::gcsd)->minDamage;
		}
		if (damage < (*fo::ptr::main_ctd).targetDamage) { // check the hit points and set the DAM_DEAD flag
			fo::func::check_for_death((*fo::ptr::main_ctd).target, (*fo::ptr::main_ctd).targetDamage, &(*fo::ptr::main_ctd).targetFlags);
		}

		if ((*fo::ptr::main_ctd).targetDamage > (*fo::ptr::gcsd)->maxDamage) {
			(*fo::ptr::main_ctd).targetDamage = (*fo::ptr::gcsd)->maxDamage;
		}
...
static long __fastcall MultiHexPlacement(fo::GameObject* source) {
	...
	do {
		...
		dudeRot = (*fo::ptr::obj_dude)->rotation;
	} while (--count);
...
static long __fastcall TargetIsActiveForPush(fo::GameObject* source, fo::GameObject* target) {
	...
	if (target->critter.damageFlags & fo::DamageFlag::DAM_KNOCKED_DOWN && *fo::ptr::combat_state & fo::CombatStateFlag::InCombat) return 0;
...
void BugFixes::init() {
	...
	//if (IniReader::GetConfigInt("Misc", "NPCDrugAddictionFix", 1)) {
		...
		HookCalls(item_d_take_drug_hook, itemTakeDrugHkAddr); // replace item_d_check_addict_ function calls with sfall implementation
	...
	const DWORD combatLoadFindObjAddr[] = {0x42113B, 0x42117D};
	HookCalls(combat_load_hook_critter, combatLoadFindObjAddr);
	...
	const DWORD displayInvenInfoAddr[] = {
	...
	MakeCalls(display_inventory_hack_info, displayInvenInfoAddr); // when displaying the inventory list
	...
	const DWORD charEditRegBtnSndAddr[] = {0x433901, 0x433966};
	SafeWriteBatch<WORD>(0x9090, charEditRegBtnSndAddr); // remove incorrect button ID assignment (CharEditStart_)
