...
static const long WorldMapHealingDefaultInterval = 180 * 60 * 10; // 3 hrs
...
static void PathfinderFixInit() {
	//if (IniReader::GetConfigInt("Misc", "PathfinderFix", 1)) {
...
static void StartingStatePatches() {
	...
	if (date >= 0) {
		...
		if (date >= fo::ptr::days_in_month[month]) date = fo::ptr::days_in_month[month] - 1;
