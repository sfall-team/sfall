...
void __stdcall CombatAI::ai_check_drugs(fo::GameObject* source) {
	...
	if (!lastItem) {
		...
		case fo::AIpref::CHEM_stims_when_hurt_little: // use only healing drugs
			hpPercent = 60;
			break;
		case fo::AIpref::CHEM_stims_when_hurt_lots:   // use only healing drugs
			hpPercent = 30;
			break;
		case fo::AIpref::CHEM_sometimes:
			if (!(*fo::ptr::combatNumTurns % 3)) chance = 25; // every three turns
			//hpPercent = 50;
			break;
		case fo::AIpref::CHEM_anytime:
			if (!(*fo::ptr::combatNumTurns % 3)) chance = 75; // every three turns
			//hpPercent = 50;
			break;
		case fo::AIpref::CHEM_always:
			chance = 100; // 99%
			break;
		case fo::AIpref::CHEM_clean:
