...
static std::tr1::array<sfChild, 2000> m_pathData;
static std::tr1::array<sfChild, 2000> m_dadData;
static std::tr1::array<BYTE, 5000> seenTile;
...
// optimized version with added 'type' arg    NOTE: REMEMBER TO CHANGE fo::func::tile_coord TO STDCALL IF IMPLEMENTING THIS!
...
long __fastcall Tilemap::make_path_func(fo::GameObject* srcObject, long sourceTile, long targetTile, long type, void* arrayRef, long checkTargetTile, void* blockFunc) {
	...
	bool inCombat = *fo::ptr::combat_state & CBTFLG_InCombat;
	...
	std::tr1::array<sfChild, 2000>::iterator& it = m_pathData.begin();
	it->tile = sourceTile;
	it->from_tile = -1;
	...
	std::tr1::array<sfChild, 2000>::iterator& dadData = m_dadData.begin();
	size_t pathCounter = 1;
	sfChild childData;

	// search path tiles
	while (true) {
		std::tr1::array<sfChild, 2000>::iterator& pathData = m_pathData.begin();
		if (pathCounter > 0) {
			size_t counter = 0;
			long prevDistance;

			// search for the element with the smallest distance
			for (std::tr1::array<sfChild, 2000>::iterator currData = pathData; currData != m_pathData.end(); ++currData) {
				if (currData->tile != -1) {
